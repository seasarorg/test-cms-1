...
import org.seasar.framework.util.ClassLoader24Util;
import org.seasar.framework.util.ClassUtil;
import org.seasar.framework.util.Disposable;
import org.seasar.framework.util.DisposableUtil;
import org.seasar.framework.util.JarFile24Util;
import org.seasar.framework.util.JarFileUtil;
import org.seasar.framework.util.MapUtil;
import org.seasar.framework.util.ResourceUtil;
import org.seasar.framework.util.String24Util;
import org.seasar.framework.util.StringUtil;
import org.seasar.framework.util.URL24Util;
...
public class NamingConventionImpl implements NamingConvention, Disposable {
    ...
    public Class toCompleteClass(Class clazz) {
        ...
        if (ResourceUtil.isExist(Class24Util.getResourcePath(className))) {
            return ClassUtil.forName(className);
        }
        return clazz;
    }
    ...
    public boolean isTargetClassName(final String className, final String suffix) {
        ...
        if (!String24Util.trimSuffix(className, implementationSuffix).endsWith(
                suffix)) {
            return false;
        }
        final String shortClassName = Class24Util.getShortClassName(className);
        if (className.endsWith(implementationSuffix)
    ...
    protected ExistChecker[] createExistCheckerArray(
        ...
        for (final Iterator it = ClassLoader24Util.getResources(
                this.getClass(), s); it.hasNext();) {
            final URL url = (URL) it.next();
            final String protocol = URL24Util.toCanonicalProtocol(url
                    .getProtocol());
    ...
    protected static class FileExistChecker implements ExistChecker {
        ...
        protected FileExistChecker(final URL rootUrl) {
            rootFile = URL24Util.toFile(rootUrl);
        }
